% Generates color coded distance matrices for visualization of each
% correlational matrix

load("allDistanceMatrices_1.mat", 'allMatrices') %final matrices file
matrix = struct2cell(allMatrices);
matrixNames = fieldnames(allMatrices);

for f=1:length(matrix) % generates images from finalMat.mat file
    letterTriTrans = transpose(matrix{k}); %flips the matrix so triangle is at the bottom
    heatmap(letterTriTrans, 'Colormap', parula)
    set(gca,'XData',wordlist, 'YData', wordlist)
    set(gca, 'FontSize', 4)
    title(matrixNames{k})
%     pcolor(wordlistOutput(:,k))
%     colorbar % makes color coded image
    sheetNum = append('Sheet',num2str(k)); % new sheet number
    xlswritefig(gcf, 'distanceMatrix.xlsx', sheetNum, 'A1'); % export file to Sheet n, in cell A1 on Excel
end
